# Phase 3 - RoBERTa Binary Classification (FIXED - No Data Leakage + Live Training Graphs)
# This version removes data leakage, uses full datasets, includes proper validation, and live plotting
...
# Reduce TF oneDNN/INFO verbosity
...
class LivePlotCallback(TrainerCallback):
    ...
    def _update_plots(self):
        """Update all subplot visualizations."""
...
def retrieve_top_k_contexts(collection, query_text, k=3, exclude_product_id=None, 
    ...
    """Query the Chroma collection and return up to k context strings."""
...
def precompute_rag_for_examples(examples, collection, out_path, key_review='review_text',
    ...
    """Precompute retrieved contexts for examples and save to cache."""
    if examples is None:
        return {}
...
def main():
    ...
    # Merge product info with test set
    ...
    # Normalize test_df and create stable IDs
    ...
    # CRITICAL FIX: Remove train-test overlap BEFORE deduplication
    ...
    # Step 1: Remove exact text matches first (highest priority)
    ...
    # Step 2: Remove normalized text matches (catches paraphrases)
    ...
    # CRITICAL CHECK: Ensure both classes still exist
    ...
    # Deduplicate AFTER overlap removal
    df = df_clean.copy()
    before_dedup = len(df)
    ...
    # CRITICAL FIX: Perform stratified split with minimum validation checks
    print(f"\n{'='*70}")
    ...
    # Create deduplicated examples list
    ...
    # Callbacks (UPDATED WITH LIVE PLOTTING)
    callbacks = []
    
    # Add live plotting callback
    try:
